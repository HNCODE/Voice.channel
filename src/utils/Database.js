...
class Database {
  ...
  async connect (re = false) {
    ...
    try {
      ...
      if (this.maxReconnectTries <= this.reconnectTries) {
        ...
        return new Error(`${this.defaultPrefix.init} Failed to ${msg} MongoDB! (${this.reconnectTries} Tries)`)
      }
  ...
  async getGuild (guildId) {
    ...
    if (!result) {
      ...
      try {
        ...
        return new Error(`${this.defaultPrefix.getGuild} Failed to create guild data via guildId: ${guildId}`)
      }
    }
    return result
  }
  ...
  async getMember (memberId, guildId) {
    ...
    if (!result) {
      ...
      try {
        ...
        return new Error(`${this.defaultPrefix.getMember} Failed to create guild data via memberId: ${memberId} & guildId: ${guildId}`)
      }
    }
    return result
  }
  ...
  async getUser (userId) {
    ...
    if (!result) {
      ...
      try {
        ...
        return new Error(`${this.defaultPrefix.getUser} Failed to create guild data via userId: ${userId}`)
      }
    }
    return result
  }
  ...
  async addErrorInfo (type = 'default', error, message, command, args) {
    ...
    try {
      ...
      return new Error(`${this.defaultPrefix.addErrorInfo} Failed to create errorInfo data via errorInfo UUID: ${createUUID}`)
    }
