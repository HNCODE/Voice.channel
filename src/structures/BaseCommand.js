class BaseCommand {
  constructor (client, name, aliases, category, permissions, usage, description, hide, requirements) {
    this.client = client
    this.name = name
  ...
  replaceHolder (commandName = this.name, text) { return text.replace(/%PREFIX%/gi, this.client._options.bot.prefix).replace(/%COMMAND%/gi, commandName) }
